    ...
    for b = 1:num_blocks
        ...
        for image = imgStartIndex:imgEndIndex % self condition
            ...
            if behavData.Data.choice(image) == "stand" || behavData.Data.choice(image) == "fall"
                ...
                save_img_dir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/5_motconSubjectMatrices/sub-%s/mot-%s/", sub, mot)));
                save_img_path = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/5_motconSubjectMatrices/sub-%s/mot-%s/", sub, mot));
                ...
                if isfile(sprintf("%simage-%s.csv", save_img_path, save_name(2))) == 0
                    ...
                    writematrix(img_heatmap, sprintf("%simage-%s.csv", save_img_path, save_name(2)));
                end
                
                % if it already exists, add each element to the existing matrix
            else
                disp("Data for this image already exists!");
                
                if isempty(d) == 0
                    
                    old_heatmap = readmatrix(sprintf("%simage-%s.csv", save_img_path, save_name(2)));
                    ...
                    % add to old file
                    writematrix(combined_heatmap, sprintf("%simage-%s.csv", save_img_path, save_name(2)));
