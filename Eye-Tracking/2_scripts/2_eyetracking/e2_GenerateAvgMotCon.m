...
for i = 1:length(Sub)
    ...
    for b = 1:num_blocks
        ...
        for image = imgStartIndex:imgEndIndex % self condition
            ...
            if behavData.Data.motivation(image) == "stand" && behavData.Data.choice(image) == "stand"
                mot = "con";
            elseif behavData.Data.motivation(image) == "fall" && behavData.Data.choice(image) == "fall"
                mot = "con";
            elseif behavData.Data.motivation(image) == "fall" && behavData.Data.choice(image) == "stand"
                mot = "incon";
            elseif behavData.Data.motivation(image) == "stand" && behavData.Data.choice(image) == "fall"
                mot = "incon";
            else
                mot = "neutral";
            end
            
            if behavData.Data.choice(image) == "stand" || behavData.Data.choice(image) == "fall"
                
                this_arr = [posX_arr posY_arr];
                
                save_img_dir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/mot-%s/", mot)));
                save_img_path = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/mot-%s/", mot));
...
mots = ["mot-con", "mot-incon", "mot-neutral"];

sum_files = dir('../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/mot-neutral/');
...
for i=1:length(sum_files)
    ...
    for j=1:3
        ...
        mot = mots(j);
        d = readmatrix(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/%s/%s", mot, sum_files(i).name));
        ...
        save_img_dir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/%s/", mot)));
        save_img_path = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/1_heatmapMatrices/6_motconAverageMatrices/%s/", mot));
