...
for i = 1:length(inp_files)
    ...
    for j = 1:length(Sub)
       ...
       if isfile(sprintf('%ssub-%s/mot-fall/%s', indir, sub, inp_files(i).name))
          ...
          outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot/mot_same/mot-fall/%s/", inp_files(i).name)));
          isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot/mot_same/mot-fall/%s/", inp_files(i).name));
       ...
       elseif isfile(sprintf('%ssub-%s/mot-stand/%s', indir, sub, inp_files(i).name))
          ...
          outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot/mot_same/mot-stand/%s/", inp_files(i).name)));
          isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot/mot_same/mot-stand/%s/", inp_files(i).name));
