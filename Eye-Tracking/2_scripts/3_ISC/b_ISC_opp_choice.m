...
% participant's eye-gaze pattern with the eye-gaze pattern of all participants who made the other choice
    ...
    for j = 1:length(Sub)
        ...
        if isfolder(sprintf('%ssub-%s/mot-neutral/', indir, sub))
            ...
            if isfile(sprintf('%schoice-fall.csv', imgpath))
                ...
                % compute correlation for the data of each subject and the
                % average of the participants who made the other choice
                this_sub_img_corr = corr(stand_vector, this_sub_vector);
            
                outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_incon/mot-neutral/%s/choice-fall/", inp_files(i).name)));              
                isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_incon/mot-neutral/%s/choice-fall/", inp_files(i).name));
                ...
                same_corr = table2array(readtable(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_con/mot-neutral/%s/choice-fall/%s.txt", inp_files(i).name, sub))));
                
                compare_corr = same_corr - this_sub_img_corr;
                
                compare_outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_compare/mot-neutral/%s/choice-fall/", inp_files(i).name)));              
                final_isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_compare/mot-neutral/%s/choice-fall/", inp_files(i).name));
                writematrix(compare_corr, sprintf('%s%s.txt', final_isc_matrix, sub));
            ...
            elseif isfile(sprintf("%schoice-stand.csv", imgpath))
                ...
                outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_incon/mot-neutral/%s/choice-stand/", inp_files(i).name)));              
                isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_incon/mot-neutral/%s/choice-stand/", inp_files(i).name));
                
                % save the matrix with the correlation coefficients in the corresponding folder
                writematrix(this_sub_img_corr, sprintf('%s%s.txt', isc_matrix, sub));
                
                same_corr = table2array(readtable(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_con/mot-neutral/%s/choice-stand/%s.txt", inp_files(i).name, sub))));
                
                compare_corr = same_corr - this_sub_img_corr;
                 
                compare_outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_compare/mot-neutral/%s/choice-stand/", inp_files(i).name)));              
                final_isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_choice/choice_compare/mot-neutral/%s/choice-stand/", inp_files(i).name));
                writematrix(compare_corr, sprintf('%s%s.txt', final_isc_matrix, sub));
