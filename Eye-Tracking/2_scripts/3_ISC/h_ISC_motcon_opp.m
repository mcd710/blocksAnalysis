...
for i = 1:length(inp_files)
    ...
    for j = 1:length(Sub)
       ...
       if isfile(sprintf('%ssub-%s/mot-con/%s', indir, sub, inp_files(i).name))
          ...
          outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot_v2/mot_opp/mot-con/%s/", inp_files(i).name)));
          isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot_v2/mot_opp/mot-con/%s/", inp_files(i).name));
       ...
       elseif isfile(sprintf('%ssub-%s/mot-incon/%s', indir, sub, inp_files(i).name))
          ...
          outdir = mkdir(fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot_v2/mot_opp/mot-incon/%s/", inp_files(i).name)));
          isc_matrix = fullfile(sprintf("../../3_results/1_et_processing/3_heatmaps/2_isc/isc_mot_v2/mot_opp/mot-incon/%s/", inp_files(i).name));
